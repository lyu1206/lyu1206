...
namespace MessagePack.Unity.Editor
{
    internal class MessagePackWindow : EditorWindow
    {
        ...
        async void OnGUI()
        {
            ...
            if (GUILayout.Button("Generate"))
            {
                ...
                {
#if UNITY_EDITOR_OSX
                    var log = await ProcessHelper.InvokeProcessStartAsync("../Tools/mpc", "mpc " + commnadLineArguments);
                    var req = ShellHelper.ProcessCommand("python3 ../../Tools/mpc_helper.py -p " + mpcArgument.Output,
                        Application.dataPath);
#else
                var log = await ProcessHelper.InvokeProcessStartAsync("dotnet", "mpc " + commnadLineArguments);
                    var req = ShellHelper.ProcessCommand("python ../Tools/mpc_helper.py -p " + mpcArgument.Output,
                        Application.dataPath);
#endif
                    req.onLog += delegate(int logType, string shelllog) { UnityEngine.Debug.Log(shelllog); };
                    req.onDone += () => { AssetDatabase.ImportAsset("Assets/" + mpcArgument.Output); };
                    UnityEngine.Debug.Log(log);
                }
                finally
                {
    ...
    internal static class ProcessHelper
    {
        ...
        #else
        public const string DotnetPath = "dotnet";
        #endif
